...
void Genvexv2Climate::control(const climate::ClimateCall& call) {
  ...
  if (new_custom_fan_mode != nullptr && strlen(new_custom_fan_mode) > 0)
  {
...
climate::ClimateTraits Genvexv2Climate::traits() {
  ...
  traits.add_feature_flags(climate::ClimateTraits::FEATURE_FLAG_CURRENT_TEMPERATURE);
...
void Genvexv2Climate::genvexv2fanspeed_to_fanmode(const int state)
{
  ...
  case 1:
    ...
    this->set_custom_fan_mode(esphome::to_string(state).c_str());
    break;
  case 2:
    ESP_LOGD("TAG", "Case 2");
    this->mode = climate::CLIMATE_MODE_HEAT_COOL;
